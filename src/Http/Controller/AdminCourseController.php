...
final class AdminCourseController extends BaseController
{
    ...
    /**
     * The standalone Courses Administration section.
     *
     * The payload comes from the same section loader the rest of Administration uses, because
     * partials/admin/courses.html is shared with the consolidated workspace and reads the full
     * paginated view contract. Assembling a bespoke payload here left @courses_pagination and
     * its siblings undefined, and F3 turns an undefined template variable into a 500 for the
     * entire page.
     */
    public function index(): void
    {
        $user = $this->requirePermission('COURSE.MANAGEMENT.VIEW');
        $platformWide = $user->hasPermission('PLATFORM.DASHBOARD.VIEW');
        $request = [
            'courses_page' => $_GET['courses_page'] ?? null,
            'courses_page_size' => $_GET['courses_page_size'] ?? null,
        ];
        $capabilities = ['view_all_courses' => $platformWide];
        $crossUniverse = $this->canSelectUniverse($user);
        // Courses is one of the required universe surfaces, and this is its standalone route, so
        // it must offer the same selector and counts the consolidated workspace does rather than
        // leaving the reader to edit the query string.
        ...
        $this->render('admin-section', $data + [
            ...
            'page_title' => 'Courses',
            'page_kicker' => $platformWide ? 'Platform administration' : 'Course authoring',
            'admin_section' => $this->adminSections->get('courses'),
