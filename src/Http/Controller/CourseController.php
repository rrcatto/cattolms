...
final class CourseController extends BaseController
{
    ...
    {
        $this->requireCsrf();
        $user = $this->requirePermission('CATALOGUE.FAVOURITE');
    ...
    {
        $this->requireCsrf();
        $user = $this->requirePermission('CATALOGUE.REQUEST');
        $courseId = max(1, (int) ($_POST['course_id'] ?? 0));
    ...
    public function media(): void
    {
        ...
        if (!$allowed && $user !== null) {
            $allowed = $this->courses->userCanAccessMedia($user->id, $media, $user->hasRole('ADMIN'));
        }
        if (!$allowed) {
