...
final class CoursePortabilityService
{
    ...
    {
        $source = $this->requireCourse($courseId);
    ...
    private function importAnalysis(array $analysis, int $categoryId, int $userId, ?int $replaceCourseId): int
    {
        ...
        if ($replaceCourseId !== null) {
            $existing = $this->requireCourse($replaceCourseId);
    ...
    /** @param array<string,mixed> $course @param array<string,string> $values */
    ...
    private function writeCourseBackup(int $courseId, string $reason): string
    {
        $course = $this->requireCourse($courseId);
        $directory = $this->storageRoot . '/exports/course-backups';
        $this->requireDirectory($directory);
        $slug = preg_replace('/[^a-z0-9_-]+/i', '-', (string) ($course['slug'] ?? 'course-' . $courseId)) ?: 'course-' . $courseId;
        $path = $directory . '/' . $slug . '-' . gmdate('Ymd-His') . '-' . $reason . '.course.json';
        if (file_put_contents($path, $this->exportCourseJson($courseId), LOCK_EX) === false) {
            throw new RuntimeException('The automatic course backup could not be written. No destructive action was performed.');
        }
        @chmod($path, 0640);
        return $path;
    }
