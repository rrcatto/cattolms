        ...
        $rows = match ($type) {
            'people' => $this->db->exec(
                'SELECT u.id,
                 ...
                 WHERE u.id=:id AND (:company_id=0 OR cu.company_id=:company_id)
                 LIMIT 1',
                [':id' => $id, ':company_id' => $companyId]
            ),
            'companies' => $this->db->exec(
                'SELECT c.id, c.name AS label, c.domain AS detail FROM companies c WHERE c.id=:id LIMIT 1',
                [':id' => $id]
            ),
            'courses' => $this->db->exec(
                'SELECT c.id, c.title AS label, c.slug AS detail
                 FROM courses c
                 WHERE c.id=:id AND (:company_id=0 OR c.owner_company_id=:company_id)
                 LIMIT 1',
                [':id' => $id, ':company_id' => $companyId]
            ),
            default => [],
