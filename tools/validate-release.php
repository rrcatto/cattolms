...
$need(($composer['version'] ?? '') === '0.5.7.6', 'composer.json version must be 0.5.7.6.');
...
$need(!str_contains($baseline, 'CREATE TABLE seed_data') && preg_match('/^\\s*seed_token\\s+/m', $baseline) !== 1, 'v0.5.7.6 must not implement Seed Database schema yet.');
...
foreach (["PLATFORM_ASSET_VERSION = '0.5.7.6'","set('navigation'","set('footer_navigation'","\$admin['sections']","\$admin['section']","\$account['sections']","\$account['section']","\$company['sections']","\$company['section']",'$this->adminSections->all()','$this->accountSections->all()','$this->companySections->all()'] as $token) {
...
$need(in_array('factory-reset-v1.0.1.zip', $bundledNames, true), 'extras/themes must include the default factory-reset-v1.0.1.zip package.');
...
echo 'Release validation passed: Catto Learning 0.5.7.6, ' . count($pages) . " platform pages, simplified ACL/workspace/theme-sync contracts, PHP 8.5.9 target.\n";