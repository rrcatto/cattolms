...
Date time: 2026/08/19 02:34 SAST
Version: 0.5.7.4
Description:
Validates the v0.5.7.4 presentation contract: core-owned interactions, stable standard navigation/footer arrays, consolidated Account/Company/Administration section APIs, ACL UI, scalable Companies/Activity views and Factory Reset Theme Package 3.0 behaviour.
*/
...
foreach (['.cl-admin-workspace{display:grid;','.cl-account-workspace,.cl-company-workspace','.activity-filters{width:100%','.pagination-row{display:flex','.acl-grid{display:grid','.modal-backdrop[hidden]','.cl-palette-switcher'] as $token) {
...
foreach (['name="permissions[]"','Save permissions'] as $token) $need(str_contains($roleEdit, $token), 'ACL editor missing: ' . $token);
...
foreach (["PLATFORM_ASSET_VERSION = '0.5.7.4'","set('navigation'","set('footer_navigation'",'account_sections','account_section','company_sections','company_section','admin_sections','admin_section'] as $token) {
