...
foreach (['companies' => 'companies.html', 'company_creators' => 'company-creators.html'] as $dataset => $file) {
    ...
    $need(str_contains($companies, 'partials/admin/company-group-nav.html'), $file . ' must render the Companies tab strip.');
    $need(str_contains($companies, 'partials/admin/company-modals.html'), $file . ' must include the shared company dialogs.');
}
...
foreach ([
    ...
    'group="{{ @pg.pages }}"' => 'adaptive numbered-page window',
    '{{ @pg_item.is_gap }}' => 'ellipsis for an elided range',
...
$requests = $read($root . '/resources/views/partials/admin/requests.html');
...
foreach (["PLATFORM_ASSET_VERSION = '0.6'","set('navigation'","set('footer_navigation'",'account_sections','account_section','company_sections','company_section','admin_sections','admin_section'] as $token) {
...
foreach ([['enrolments', $enrolments], ['requests', $requests]] as [$dataset, $markup]) {
    $need(
        substr_count($markup, 'partials/universe-switch.html') === 1,
