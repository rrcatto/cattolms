...
The version-specific installation and reset instructions below describe their named historical versions. They are not an upgrade procedure for the current v0.8.7 code or for a production system. Follow the current release procedure and `PROJECT-INSTRUCTIONS.md` for any later authorized deployment; do not apply a historical reset command to a populated instance.
